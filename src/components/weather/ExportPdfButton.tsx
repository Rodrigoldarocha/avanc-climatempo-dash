...
export const ExportPdfButton = ({ location }: ExportPdfButtonProps) => {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      const footerReserve = 14; // space reserved for footer at bottom
      ...
      if (currentData?.data) {
        ...
        checkPageBreak(sectionH + 6);
        drawRoundedRect(pdf, margin, y, contentWidth, sectionH, 3, "#FFFFFF", darkColor);
      ...
      if (allHours.length > 0) {
        ...
        const usableW = contentWidth - 8; // 4mm padding each side
        const hourWidth = usableW / allHours.length;
      ...
      // === 15-DAY FORECAST (with proper per-row pagination) ===
      const forecastData = Array.isArray(dailyData?.data) ? dailyData.data.slice(0, 15) : [];
      
      if (forecastData.length > 0) {
        const rowHeight = 6;
        const headerH = 8;
        const tableHeaderH = 5;
        const sectionTitleH = headerH + tableHeaderH + 2; // title bar + column headers + gap
        ...
        const drawSectionHeader = () => {
          drawRoundedRect(pdf, margin, y, contentWidth, headerH, 2, darkColor);
          pdf.setFontSize(9);
          pdf.setFont("helvetica", "bold");
          pdf.setTextColor("#FFFFFF");
        ...
        const drawColumnHeaders = () => {
          ...
          pdf.setFont("helvetica", "bold");
          pdf.setTextColor(mediumGray);
          pdf.text("DATA", cols.date, y + 3);
          pdf.text("CONDIÇÃO", cols.condition, y + 3);
          pdf.text("MÍN", cols.min, y + 3);
          pdf.text("MÁX", cols.max, y + 3);
          pdf.text("CHUVA", cols.rain, y + 3);
          y += tableHeaderH + 1;
        };

        // Initial section header
        checkPageBreak(sectionTitleH + rowHeight);
        drawSectionHeader();
        drawColumnHeaders();

        forecastData.forEach((day: any, index: number) => {
          // Check if this row fits; if not, break to new page and redraw headers
          if (y + rowHeight > maxY) {
            pdf.addPage();
            y = margin;
            drawSectionHeader();
          ...
          pdf.setTextColor(textColor);
          pdf.text(dateLabel, cols.date, y + 3);

          pdf.setFont("helvetica", "normal");
          const conditionText = getConditionText(icon).substring(0, 22);
          pdf.text(conditionText, cols.condition, y + 3);

          pdf.setTextColor("#3B82F6");
          pdf.text(`${Math.round(minTemp)}°C`, cols.min, y + 3);
          
          pdf.setTextColor("#F97316");
          pdf.text(`${Math.round(maxTemp)}°C`, cols.max, y + 3);

          pdf.setTextColor(rainProb > 50 ? "#3B82F6" : mediumGray);
          pdf.text(`${rainProb}%`, cols.rain, y + 3);
      ...
      if (forecastData.length > 0) {
        ...
        drawRoundedRect(pdf, margin, y, contentWidth, summaryH, 3, lightGray);
