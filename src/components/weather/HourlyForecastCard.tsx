  ...
  try {
    if (data?.data && Array.isArray(data.data)) {
      ...
      allHours = data.data.map((item: any) => {
        ...
        const dateBr = String(item.date_br || "");
        ...
        if (typeof item.rain === 'object' && item.rain !== null) {
          rainValue = Number(item.rain.precipitation ?? 0);
        ...
        if (typeof item.humidity === 'object' && item.humidity !== null) {
          humidityValue = Number(item.humidity.humidity ?? 0);
        ...
        return {
          ...
          date_br: dateBr,
          hour,
          temp: Number(item.temperature?.temperature ?? item.temp ?? 0),
          humidity: humidityValue,
          rain: rainValue,
          wind_direction: String(item.wind?.direction ?? item.wind_direction ?? ""),
          wind_velocity: Number(item.wind?.velocity ?? item.wind_velocity ?? 0),
        };
      });
    } else if (data?.data && typeof data.data === 'object') {
      // Old format: object with hour_to_hour array
      const dataObj = data.data as Record<string, any>;
      if (Array.isArray(dataObj.hour_to_hour)) {
        allHours = dataObj.hour_to_hour.map((hour: any) => ({
          date: String(dataObj.date || ""),
          date_br: String(dataObj.date_br || ""),
          hour: String(hour.hour || ""),
          temp: Number(hour.temp ?? 0),
          humidity: Number(hour.humidity ?? 0),
          rain: Number(hour.rain ?? 0),
          wind_direction: String(hour.wind_direction ?? ""),
          wind_velocity: Number(hour.wind_velocity ?? 0),
        }));
      }
    }
  } catch (parseError) {
    console.error("HourlyForecastCard parse error:", parseError);
    allHours = [];
  }
