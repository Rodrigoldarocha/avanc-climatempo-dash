...
export const ExportDataButton = ({ location }: ExportDataButtonProps) => {
  ...
  const downloadExcel = (filename: string) => {
    // Build a single CSV with all sections separated by headers
    const sections: string[] = [];

    const currentRows = buildCurrentRows();
    if (currentRows.length > 0) {
      sections.push(rowsToCsv(currentRows, "Clima Atual"));
    }

    const hourlyRows = buildHourlyRows();
    if (hourlyRows.length > 0) {
      sections.push(rowsToCsv(hourlyRows, "Previsão 72h"));
    }

    const dailyRows = buildDailyRows();
    if (dailyRows.length > 0) {
      sections.push(rowsToCsv(dailyRows, "Previsão 15 dias"));
    }
  ...
  const rowsToCsv = (rows: Record<string, any>[], sectionTitle: string) => {
    const headers = Object.keys(rows[0]);
    return [
      `--- ${sectionTitle} ---`,
      headers.join(";"),
    ...
    try {
      ...
      switch (format) {
        ...
        case "csv-daily":
          ...
          break;
        case "excel":
          downloadExcel(`relatorio-completo-${citySlug}-${date}.csv`);
          break;
      }
      ...
      <DropdownMenuContent align="end" className="w-52">
        ...
        <DropdownMenuItem onClick={() => handleExport("excel")}>
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-muted-foreground">Todas as séries em .csv</span>
          </div>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
